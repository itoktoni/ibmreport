    ...
    <table id="datatable" class="responsive table table-no-more table-bordered table-striped mb-none">
        <thead>
            <tr>
                ...
                <th class="text-left" style="width:12%">No. Spk</th>
                <th class="text-left" style="width:12%">Tgl SPK</th>
                <th class="text-left" style="width:20%">Nama Penjahit</th>
        ...
        @php
            ...
            if($has_detail){
                ...
                foreach($has_detail as $detail){
                    $total_detail = $detail->qty_receive * $detail->price;
            ...
            <tr>
                ...
                <td data-title="No. Order">{{ $data->spk_id ?? '' }} </td>
                <td data-title="Tgl Spk">{{ $data->spk_date ?? '' }} </td>
                <td data-title="Nama Penjahit">{{ $data->has_production->mask_name ?? '' }} </td>
                <td class="text-right" data-title="Tgl Bayar">{{ $tgl_bayar }} </td>
                <td class="text-right" data-title="Status">{{ $data->spk_status }} </td>
